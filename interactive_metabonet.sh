...
echo "| \"Compartment and Hub Denitions Tune MetabolicNetworks for Metabolomic        |"
echo "| Experiments\".                                                                |"
echo "|                                                                              |"
echo "+------------------------------------------------------------------------------+" 2>/dev/null
...
then
  ...
  METABONET=$REPLY
  echo "| MetaboNet directory now set to $METABONET"
fi
...
echo "| Working directory for inputs and outputs currently set to $DOCK"
...
then
  ...
  read -p "| Please provide the full path to the MetaboNet directory: " -r
  DOCK=$REPLY
  echo "| Working directory now set to $DOCK"
fi
...
then
  ...
  cp -r $DOCK/source/reconciliation_2019-08-13/ $DOCK/reconciliation/
...
echo "| Step 5.1: Compartments true, Hubs true "
...
echo "| Step 5.2: Compartments true, Hubs false "
...
echo "| Step 5.3: Compartments false, Hubs true "
...
echo "| Step 5.4: Compartments false, Hubs false "
